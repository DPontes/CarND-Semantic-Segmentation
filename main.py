...
# Used for plotting to vizualize if our training is going well
all_training_losses = []

# Dependency Check - Tensorflow
assert LooseVersion(tf.__version__) >= LooseVersion('1.0'), 'Please user tensorflow version 1.0 or newer. You are using {}'.format(tf.__version__)
print('Tensorflow version: {}'.format(tf.__version__))

# Dependency Check - GPU
...
def load_vgg(sess, vgg_path):
    ...
    vgg_path: Path to vgg folder, contaning 'variables/' and 'saved_model.pb'
    return: tuple of tensors from VGG model (imafge_input, keep_prob, layer3, layer4, layer7)
    """
    
    # Load the model and weights
    model = tf.saved_model.loader.load(sess, ['vgg16'], vgg_path)
    
    # Get tensors to be returned from grapth
...
def upsample(layer, k, s, layer_name):
    # Return the output of a transpose convolution given kernel _size k and strides s
...
def layers(vgg_layer3_out, vgg_layer4_out,vgg_layer7_out, num_classes = NUMBER_OF_CLASSES):
    ...
    # changing the name of the variable for simplicity
    ...
    # Add decoder layers to the network with skip connectons and upsampling
...
def optimize(nn_last_layer,correct_label, learning_rate, num_classes = NUMBER_OF_CLASSES):
    ...
    correct_label: TF placeholder of the correct label image
...
def run():
    """
    Run a train model and save output images resulting from the test image feed on the trained model
    """
    
    # Get the vgg model if we can't find it where it should be
    ...
    with tf.Session() as session:
        ...
        # Return the three layers, keep probability and input layer from the VGG architecture
        ...
        # Run the model with the test images and save each painted output image
